...
## Pitfalls — the "floor view" and slanted-side traps

- **Don't use `ThreeDScene` for 2D content.** Text, code, captions, diagram
  images, and flat bars are 2D. Putting them in a `ThreeDScene` forces a
  perspective "floor view" that tilts flat elements. Use a plain `Scene` for
  those; keep `ThreeDScene` for genuinely 3D geometry/surfaces.
- **Center 3D props; don't push them to the sides.** A `Cube`/`Prism` placed at
  `x = ±3.6` with a 3/4 camera looks "slanted at the side." Keep 3D accents near
  `frame_center=[0,0,0]`.
- **Avoid a head-on `theta`.** `set_camera_orientation(phi=70*DEGREES, theta=30*DEGREES)`
  is the balanced 3/4. A near-head-on `theta` (≈ -20°/0°) combined with
  side-placed 3D objects is exactly what produces the bad slanted look.
- **`add_fixed_in_frame_mobjects(mob)` pins 2D mobjects to the screen — do NOT
  follow it with `self.remove(mob)`** (that removes the object entirely). Animate
  it in with `self.play(FadeIn(mob))` instead. This helper does not flatten 3D
  props; use `Rectangle`/`Square` for flat shapes.
