...
## Agent install flow (npx-style: zero manual setup)

1. Ensure `uv` is present:
   ```bash
   command -v uv >/dev/null 2>&1 || curl -LsSf https://astral.sh/uv/install.sh | sh
   ```
2. From the `tgforwarder/` repo root:
   ```bash
   uv tool install . --no-cache
   ```
   → `tgf` on PATH (`~/.local/bin/tgf`).
3. **Write `.env` locally — NEVER commit it. Secrets only.** Required keys:
   ...
   SOURCE_CHANNELS=-100...
   DEST_CHANNELS=-100...,1255087768      # 1255087768 = your Saved Messages user id
...
```bash
tgf forward --source 558372819 --dest 1255087768 --order oldest --all --delay 3
# or interactive:
tgf forward
```

- `--dest 1255087768` = the user's **Saved Messages** (replace with their real id).
...
- `git push` to github.com **hangs from this host** — use `git bundle` and push from egress.
