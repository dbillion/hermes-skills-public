...
## Config Schema Pitfall — READ THIS FIRST

The config.json schema is strict. Common mistakes that cause crashes:

**Mistake 1: Missing `profile` field**
```json
// ❌ WRONG — crashes with KeyError: 'profile'
{ "slides": [...], "theme": "dark" }

// ✅ CORRECT — profile is REQUIRED
{ "profile": { "name": "Display Name", "handle": "@handle", "verified": true }, "slides": [...] }
```

**Mistake 2: Using `text` instead of `tweets` in slides**
```json
// ❌ WRONG — script expects tweets array
{ "slides": [{ "text": "Hello" }] }

// ✅ CORRECT — slides.tweets is an array of tweet objects
{ "slides": [{ "tweets": [{ "text": "Hello", "image": null }] }] }
```

**Mistake 3: Using `headshot` path that doesn't exist**
- If `headshot` is `null` or missing → gray circle placeholder (OK, no crash)
- If `headshot` is a path that doesn't exist → crash
- When in doubt, omit `headshot` or set it to `null`

**Minimal working config template:**
```json
{
  "profile": { "name": "Your Name", "handle": "@yourhandle", "verified": true },
  "theme": "dark",
  "slides": [
    { "tweets": [{ "text": "Your hook text here\\n\\nWith paragraph breaks.", "image": null }] }
  ]
}
```

**Script path on THIS system:**
```
~/.hermes/skills/creative/instagram-carousel/scripts/thread-to-carousel.py
```
NOT the `.claude/skills/` path shown in examples above (those reference the original Claude Code skill filesystem).
