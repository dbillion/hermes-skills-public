...
6. **Recommended default orientation** - Use a stable 3/4 view:
   `phi=70*DEGREES, theta=30*DEGREES, zoom=1.05, frame_center=[0,0,0]`.
   Keep `phi` around 70° (top-down-ish) and `theta` between -45° and +30°.
   Avoid `theta` near 0°/head-on with 3D props placed at the sides — it makes
   axis-aligned cubes/prisms look "slanted" and off-kilter.

## When NOT to use ThreeDScene (the "floor-view" pitfall)

A `ThreeDScene` projects everything through a perspective camera. If your
content is fundamentally **2D** — text, code blocks, captions, flow-diagram
images, flat bars — wrapping it in `ThreeDScene` produces a bad "floor view"
where flat elements get a fake perspective tilt. **Use a plain `Scene` for
2D explainers.** Reserve `ThreeDScene` for content that is genuinely 3D
(surfaces, real 3D geometry you rotate around).

If you must keep a `ThreeDScene` but only have a few 3D accents:
- Center the 3D props near `frame_center` (e.g. `(0,0,0)`), **not** off to
  `x = ±3.6`. Side-placed 3D objects read as "slanted at the side."
- Pin every 2D label/code/diagram to the screen with
  `self.add_fixed_in_frame_mobjects(mob)` and **do not** call `self.remove(mob)`
  afterward — removing it undoes the add and the object won't show. The correct
  pattern is:
  ```python
  self.add_fixed_in_frame_mobjects(code_m)   # pins to screen, stays in scene
  self.play(FadeIn(code_m))                   # animate it in
  ```
- Never rely on `add_fixed_in_frame_mobjects` to "flatten" a 3D prop (a `Cube`
  or `Prism`). Those stay in 3D space and take the camera's slant. Replace them
  with 2D `Rectangle`/`Square` if you want them flat.
