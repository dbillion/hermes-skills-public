...
## Free Claude Code (fcc-claude) — Multi-Provider Free Models

**free-claude-code** (`fcc-claude`) is a community project that provides a Claude Code-compatible CLI with access to **FREE models** from multiple providers: OpenRouter, NVIDIA NIM, Groq, Google Gemini, and more. Unlike the official Claude Code (Anthropic-only), fcc-claude supports **dynamic model discovery** via provider APIs.
...
### Configuration — Dynamic Model Discovery (PREFERRED)

**DO NOT hardcode model lists.** Instead, configure environment variables to enable dynamic discovery from each provider's `/v1/models` endpoint.

**Config file location:** `~/.config/free-claude-code/.env` (or set `FCC_ENV_FILE` to override)
...
# === Dynamic Model Discovery (empty string = fetch from API) ===
# OpenRouter: auto-fetch ALL free/negative-cost models
FCC_SMOKE_OPENROUTER_FREE_MODELS=""

# NVIDIA NIM: auto-fetch ALL available models (all free)
FCC_SMOKE_NIM_MODELS=""

# Groq: auto-fetch ALL available models (all free)
FCC_SMOKE_GROQ_MODELS=""

# Gemini: auto-fetch ALL available models (free tier)
FCC_SMOKE_GEMINI_MODELS=""
```

**Key insight:** Setting these vars to `""` (empty string) tells fcc-claude to call the provider's `/v1/models` endpoint and dynamically build the model list. This means:
- New models are auto-discovered when providers add them
- No config changes needed as models come/go
- You get the complete free tier, not a hardcoded subset

### Expected Model Count (Dynamic Discovery)

| Provider | Models | Cost |
|----------|--------|------|
| NVIDIA NIM | ~10-15 | All FREE (Llama 3.1 8B/70B/405B, Mistral, Mixtral, Nemotron-4 340B, etc.) |
| OpenRouter | ~50-100+ | FREE/negative-cost tier (Owl Alpha, Pareto Code, etc.) |
| Groq | ~5-10 | All FREE (Llama, Mixtral, Gemma) |
| Gemini | ~3-5 | FREE tier (1,500 requests/day each) |
| **TOTAL** | **~70-130+** | **FREE** |

### Usage

```bash
# Source config (or add to ~/.bashrc)
source ~/.config/free-claude-code.env

# List available models (tab completion works after discovery)
fcc-claude --help

# Use a specific model
fcc-claude --model openrouter/owl-alpha "review this code"
fcc-claude --model nvidia_nim/llama-3.1-405b-instruct "analyze this PR"
fcc-claude --model groq/llama-3.3-70b-versatile "explain this function"

# Use alias (if configured)
fcc-claude --model free "write a test"
fcc-claude --model nvidia-fast "quick fix"
```

### Configuration — Static Model Lists (Alternative)

If you prefer explicit control (or discovery is failing), you can hardcode model lists:
...
FCC_SMOKE_OPENROUTER_FREE_MODELS="openrouter/owl-alpha,openrouter/pareto-code,openrouter/qwen-2.5-coder-32b-instruct"
FCC_SMOKE_NIM_MODELS="nvidia_nim/llama-3.1-8b-instruct,nvidia_nim/llama-3.1-70b-instruct,nvidia_nim/llama-3.1-405b-instruct,nvidia_nim/mistral-large-2-instruct"
FCC_SMOKE_GROQ_MODELS="groq/llama-3.3-70b-versatile,groq/mixtral-8x7b-32768,groq/gemma2-9b-it"
FCC_SMOKE_GEMINI_MODELS="gemini/gemini-2.5-flash-lite-preview-06-17,gemini/gemini-2.5-flash-preview-tuning-2025-09-12"
```

**Downside:** You must manually update this list when providers add/remove models.

### Troubleshooting Model Discovery

**Problem:** Models not showing up after config change

**Solution:**
1. Ensure env vars are loaded: `env | grep FCC_SMOKE`
2. Check API keys are correct: `echo $OPENROUTER_API_KEY` (should not be empty)
3. Verify config file location: `ls -la ~/.config/free-claude-code/.env`
4. Test provider API directly:
   ```bash
   curl -H "Authorization: Bearer $NVIDIA_NIM_API_KEY" https://integrate.api.nvidia.com/v1/models | jq
   ```
5. Restart fcc-claude session (discovery happens at startup)

**Problem:** Wrong variable names

**Common mistakes:**
- `NVIDIA_API_KEY` ❌ → should be `NVIDIA_NIM_API_KEY` ✅
- `GROQ_API_KEY_1` (numbered) ❌ → should be `GROQ_API_KEY` ✅
- Keys in `~/.hermes/.env` but not sourced before running fcc-claude

**Fix:**
```bash
# Export correct names before running fcc-claude
export NVIDIA_NIM_API_KEY=$(grep NVIDIA_API_KEY ~/.hermes/.env | cut -d= -f2)
export GROQ_API_KEY=$(grep GROQ_API_KEY ~/.hermes/.env | cut -d= -f2)
# Or copy keys directly to ~/.config/free-claude-code/.env
```
...
|-----------|------------------|----------|
| Code review, refactoring | `openrouter/owl-alpha` or `nvidia_nim/llama-3.1-405b-instruct` | OpenRouter / NVIDIA |
| Fast iteration, simple fixes | `nvidia_nim/llama-3.1-8b-instruct` or `groq/llama-3.3-70b-versatile` | NVIDIA / Groq |
| Complex reasoning, planning | `nvidia_nim/nemotron-4-340b` or `openrouter/pareto-code` | NVIDIA / OpenRouter |
| CV/resume tailoring | `gemini/gemini-2.5-flash` (good formatting) | Gemini |
| Ultra-fast responses | `groq/llama-3.3-70b-versatile` (Groq speed) | Groq |

### Integration with Career-Ops

Use fcc-claude models for CV generation without burning credits:

```bash
# In ~/career-ops directory
claude -m openrouter/owl-alpha pdf --job-url "https://linkedin.com/jobs/view/123"
claude -m nvidia_nim/llama-3.1-70b-instruct html --resume cv.md --company "Stripe"
```

### Known Limitations

1. **Discovery vars must be empty string, not unset** — `FCC_SMOKE_NIM_MODELS=` or `FCC_SMOKE_NIM_MODELS=""` enables discovery; omitting the var entirely disables it
2. **API keys must match expected names** — fcc-claude uses `NVIDIA_NIM_API_KEY`, not `NVIDIA_API_KEY`
3. **Dynamic discovery happens at startup** — changing config mid-session requires restart
4. **Free tier rate limits apply** — Gemini: 1,500/day; Groq: 30 RPM / 60k tokens per request; NVIDIA: varies by model
5. **Not all OpenRouter models are free** — check `pricing` field in `/v1/models` response; fcc-claude filters for `pricing.is_free == true` or negative cost

### Pitfalls

1. **Hardcoding defeats the purpose** — if you manually list models, you miss new additions and waste time maintaining the list. Use dynamic discovery (`""`) whenever possible.
2. **Config file location mismatch** — Hermes `.env` at `~/.hermes/.env` is NOT automatically loaded by fcc-claude. Either copy keys to `~/.config/free-claude-code/.env` or source before running.
3. **Numbered env vars** — some setups use `GROQ_API_KEY_1`, `GROQ_API_KEY_2` for rotation. fcc-claude expects single `GROQ_API_KEY`. Consolidate or export the active one.
4. **Assuming all models are equal** — free tier models vary in quality and speed. Use the model selection table above for best results.
