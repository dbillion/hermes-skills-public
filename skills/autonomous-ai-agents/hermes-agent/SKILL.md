...
### Browser Toolset: What It Is and Is Not

The `browser` toolset provides **headless, text-based page interaction** through an accessibility tree snapshot. It is **not** a visual browser the user can see, and the agent **cannot** launch external browsers (Chrome, Firefox, Lightpanda, etc.) that run on the user's machine.

**What the agent can do:**
- Navigate to URLs, click elements, fill forms, and extract text content
- Return a structured text snapshot of the page
- Use it for data extraction, form submission, and web automation
- Take annotated screenshots and open them on the user's desktop with `DISPLAY=:1 xdg-open <path>`

**What the agent cannot do:**
- Display a visual browser window the user sees in real-time
- Launch local browser binaries as visible windows the user can interact with
- Access the user's already-logged-in desktop browser sessions (they run in a separate user session)

**How to communicate this to users:**
When a user asks to "open a browser" or mentions a specific browser by name, proactively clarify: *"I have built-in browser automation that lets me navigate and interact with websites textually. I can take screenshots to show you what I see, but I cannot launch a visual browser window. I'll browse the page and report back what I find."*

For users expecting a visual browser: take an annotated screenshot and open it on their desktop:
```
agent-browser --cdp 9222 screenshot --annotate --output /tmp/screenshot.png
DISPLAY=:1 xdg-open /tmp/screenshot.png
```
...
Voice messages from messaging platforms are auto-transcribed via `_enrich_message_with_transcription()` in `gateway/run.py`. Incoming voice/audio is cached to `~/.hermes/cache/audio/` (`.ogg` for voice, `.mp3` for audio), then transcribed and prepended to the user's message as: `[The user sent a voice message~ Here's what they said: "..."]`.

**Two local provider modes:**

| Mode | What it uses | Config |
|------|-------------|--------|
| `local` (default) | `faster_whisper` Python module in hermes venv | `stt.provider: local` |
| `local_command` | `whisper` CLI binary (e.g. from `uv tool install openai-whisper`) | `stt.provider: local_command` |

The `local` provider uses the `faster_whisper` Python package (already installed in the hermes venv at `~/.hermes/hermes-agent/venv`). The `local_command` provider runs the `whisper` CLI binary — useful when the Python module isn't available but the CLI is (e.g. installed via `uv tool` at `~/.local/bin/whisper`).

Provider priority (auto-detected when `stt.provider` not explicitly set):
1. **Local faster-whisper** — free, no API key
2. **Local whisper CLI** — free, uses `whisper` binary
3. **Groq Whisper** — free tier: set `GROQ_API_KEY`
...
stt:
  enabled: true
  provider: local        # local, local_command, groq, openai, mistral
  local:
    model: small         # tiny, base, small, medium, large-v3
    language: ''         # empty = auto-detect, or set 'en', 'de', etc.
```

After enabling STT or installing whisper, **restart the gateway** so Telegram voice notes are transcribed automatically.
...
print('Provider:', _get_provider(_load_stt_config()))
...
|----------|---------|-------|
| Edge TTS | None | Yes |
| Gemini TTS | `GEMINI_API_KEY` | Yes (with key) |
| ElevenLabs | `ELEVENLABS_API_KEY` | Free tier |
| OpenAI | `VOICE_TOOLS_OPENAI_KEY` | Paid |
| MiniMax | `MINIMAX_API_KEY` | Paid |
| NeuTTS (local) | None | Free |

**To send voice replies on Telegram:** Include `[[audio_as_voice]]` + `MEDIA:/path/to/audio.ogg` in the response. The gateway intercepts this and sends as a native voice bubble.

**Auto-TTS (per-chat):**
- `/voice on` — enable voice-to-voice mode (transcribe incoming, reply with voice)
- `/voice tts` — always reply with voice + text
- `/voice off` — disable auto-TTS
- Global default: `voice.auto_tts: false` in config.yaml

When auto-TTS fires, the gateway generates TTS audio FIRST, sends it as a voice message, then sends the text response.
