...
## 2.C Angular Material 3 Projects (when the stack is NOT React/Tailwind)

The dials, locks, and anti-tells above still apply, but the tooling differs. This is common: the user says "use the taste skill" on an **Angular 20 + Angular Material 3** app. Do NOT import Tailwind/shadcn/React tokens into it (that breaks the "one system per project" rule). Instead:

* **Apply the principles, not the React tooling.** Use the DESIGN_VARIANCE / MOTION_INTENSITY / VISUAL_DENSITY dials, the Color/Shape/Page-Theme locks, and the AI-tell bans (em-dash, three-equal-cards, etc.) directly in Angular templates + SCSS.
* **Theme-aware styling via Material 3 CSS variables**, not hardcoded hex. Use tokens like `--mat-sys-surface-container`, `--mat-sys-on-surface-variant`, `--mat-sys-outline-variant`. This makes light AND dark render correctly from one `ThemeService` that sets `document.documentElement.style.colorScheme`. Hardcode only the single accent (e.g. `--ks-accent: #5c6bc0`) and a soft tint (`--ks-accent-soft: rgba(92,107,192,0.12)`).
* **Motion:** GSAP for hero intro + scroll reveals (wrap in a directive using IntersectionObserver with a timeout fallback so content is visible even if the observer never fires). Respect `prefers-reduced-motion` (early-return, no animation). Never `window.addEventListener('scroll')`.
* **3D hero:** Three.js in an isolated standalone component, lazy-loaded, with a try/catch around `WebGLRenderer` so it degrades gracefully (hide canvas, keep the CSS gradient) when WebGL is unavailable. An unhandled WebGL error will break the whole page in headless/old browsers.
* **Real images:** use `https://picsum.photos/seed/{descriptive}/{w}/{h}` as the fallback when no image-gen tool is available (FAL_KEY unset -> image_gen errors with "Image generation is unavailable"). Do not fill sections with fake divs.
* **Verify with a headless browser**, not just `ng build`. Puppeteer + the system Chrome binary (`executablePath: '/usr/local/bin/google-chrome'`, `args: ['--no-sandbox','--use-gl=swiftshader']`) can load the `ng serve` URL, seed mock `localStorage` auth, and assert `pageErrors: []`, theme toggle works, and picsum images load. See `references/angular-material3-taste.md`.

The taste skill's React/Tailwind default is a DEFAULT, not a requirement. When the project is Angular Material 3, you ship the same design discipline through the project's real stack.

See `references/angular-material3-taste.md` for the concrete SCSS token recipe, GSAP-reveal directive, Three.js fallback, and Puppeteer verification script.
...
* **Styling:** **Tailwind v4** (default). Tailwind v3 only if the existing project demands it.
  ...
  * **Rails + Tailwind v4 standalone build:** When `tailwindcss-rails` gem has bundler version conflicts (common with Rails 8 + Bundler 2.x vs 4.x), bypass the gem entirely: `npm install tailwindcss @tailwindcss/cli` then `npx @tailwindcss/cli -i app/assets/stylesheets/application.tailwind.css -o app/assets/stylesheets/application.css --minify`. This produces a static CSS file served by Propshaft without needing the gem's runtime. See `references/rails-tailwind-v4.md` for full setup including Stimulus controllers for scroll-reveal, magnetic hover, and count-up animations.
...
Extended scope: multi-view Rails applications with consistent design systems across auth, dashboard, and resource views. See `references/multi-view-rails-app.md` for the full workflow including responsive sidebar→tabs, table→card, bento→stack patterns, parallel subagent building, and screenshot verification.

If the brief is data-table-heavy or pure admin panel, **say so explicitly**, point to Fluent/Carbon/Atlassian from Section 2.A, and only apply this skill's landing/marketing surfaces where they apply.
