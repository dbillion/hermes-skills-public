...
**Preflight checks (do before any setup):**
1. Confirm Mission Control is not already installed/running:
   - `ls ~/.clawdbot/mission-control.json` (config)
   - `ls ~/.clawdbot/hooks-transforms/` (webhook transform)
   - `ps aux | grep -i mission` (any running process)
2. Check Tailscale + Funnel:
   - `tailscale status`
   - `tailscale funnel status` (must not say "No serve config")
3. Check GitHub auth:
   - `gh auth status`

**External action guardrail:** enabling Tailscale Funnel, creating/pushing a GitHub repo, and registering a GitHub webhook are *external actions*. Ask the user to confirm scope (repo name + public/private) before proceeding.
