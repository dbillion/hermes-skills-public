...
`Notion-Version: 2022-06-28` is required on all HTTP requests. `ntn` handles this for you. In this version, what users call "databases" are called **data sources** in the API.
...
## Path C — Notion MCP Server (preferred for complex operations)

The official Notion MCP server (`@notionhq/notion-mcp-server`) provides 22 tools for Notion operations and is **preferred over raw API calls** for complex operations like creating pages with rich block content, databases, and structured data.

### Setup

Add to `~/.mcp_servers.json` (NOT `~/.config/mcp-cli/mcp_servers.json` — mcp-cli reads from `~/.mcp_servers.json`):
...
**Important:** The env var is `NOTION_TOKEN`, NOT `NOTION_API_KEY`. The server reads `NOTION_TOKEN` from the environment.

### Usage

Call via `mcp-cli call notion <tool_name> '<json_args>'`. For large payloads, write JSON to a file and use stdin:

```bash
# Create a page with rich block content
...
|---|---|
| Rich page creation with blocks (tables, toggles, callouts) | **MCP server** |
| Simple page with just title/text | Either MCP or curl |
| Database creation | **MCP server** |
| Quick one-off reads | curl is faster (no server startup) |
| Complex multi-step operations | **MCP server** |

### MCP Tool Discovery

Use `mcp-cli notion` (no subcommand) to list all 22 available tools. Key data tools:
- `API-post-search` — search pages/databases by title. Accepts `query` (string) and optional `sort`.
- `API-query-data-source` — query rows from a data source. Requires `data_source_id`.
- `API-retrieve-a-page` — get page metadata.
- `API-get-block-children` — list child blocks of a page or block.

**Enumerating all accessible databases**: Use `API-post-search` with an empty body to list everything the integration can see, including data sources with their IDs:

```bash
mcp-cli call notion API-post-search '{}' 2>/dev/null > /tmp/notion_search.json
```

This returns `object: "data_source"` entries with their `id` fields — the IDs you need for `API-query-data-source`. This is essential when you don't know which databases are shared with the integration.

**Querying a data source**: `API-query-data-source` reads JSON from **stdin** (not inline args). Pass the payload via pipe:

```bash
echo '{"data_source_id": "xxx"}' | mcp-cli call notion API-query-data-source 2>/dev/null
# With filter:
echo '{"data_source_id": "xxx", "filter": {"property": "Status", "select": {"equals": "Done"}}}' | mcp-cli call notion API-query-data-source 2>/dev/null
```

**Search filter syntax**: The `API-post-search` filter uses a simple structure — NOT nested `value` objects:

```
mcp-cli call notion API-post-search '{"query":"jobs"}'               # title search
```

There is no working `filter` parameter in the current MCP server build — title-only search is supported via `query`. To enumerate all databases specifically, search for titles you know and check the `object` field in results.

### Troubleshooting

- **401 Unauthorized**: Check that `NOTION_TOKEN` is set (not `NOTION_API_KEY`). Kill any cached mcp-cli daemon with `MCP_NO_DAEMON=1`.
- **Server not found**: Ensure the config is in `~/.mcp_servers.json`, not `~/.config/mcp-cli/`.
- **Tool call timeouts**: Use `MCP_NO_DAEMON=1` prefix to force fresh connections.
- **Filter returns 400**: The MCP server's `API-post-search` does not accept complex filters. Use `query` for title matching instead.
- **Pipe-to-interpreter blocked**: Some execution environments block piping CLI output directly to `python3 -c`. Write the MCP output to a temp file first, then read and parse it:
  ```bash
  mcp-cli call notion API-post-search '{"query":"jobs"}' > /tmp/result.json 2>&1
  python3 -c "import json; d=json.load(open('/tmp/result.json')); ..." 
  ```
- **mcp-cli returns zero bytes (silent failure)**: If `mcp-cli call notion <tool>` returns exit 0 but stdout AND stderr are both empty, the server is failing silently. **Skip MCP and call the Notion API directly with curl**, reading the token from `~/.mcp_servers.json`. See `references/mcp-cli-silent-failure.md` for the working fallback script.
- **Wrong API version**: Use `Notion-Version: 2022-06-28`, NOT `2025-09-03`. The newer version header may cause unexpected behavior with current integrations.

## API Version 2022-06-28 — Important Changes

- **Databases are now "data sources."** Use `/v1/databases` endpoint to create databases (not `/v1/data_sources`).
- **Database creation via API only creates the title property.** Additional properties (select, rich_text, etc.) must be added via a separate `PATCH /v1/databases/{id}` call.
...
- **Parent field requires `type`:** When creating databases, use `parent: {"type": "page_id", "page_id": "xxx"}` (the `type` field is required).
- **Internal integrations** (workspace-level) cannot create workspace-level pages. They need a specific parent page ID.
...
- **mcp-cli stdin pattern**: `API-query-data-source` and other write operations read JSON from stdin. See `references/mcp-cli-patterns.md` for the canonical command patterns.

## Cron / Scheduled Job Usage

When using this skill from Hermes cron jobs:

- **Do NOT attach this skill to cron jobs.** The SKILL.md is large enough to blow up the context window when combined with the system prompt and other skills. Instead, use direct `mcp-cli call notion <tool> '<json>'` commands in the cron prompt.
- If you need Notion data in a cron job, the MCP server (Path C) is the most token-efficient approach: `mcp-cli call notion API-post-search '{"query":"your search"}'`
- **If MCP returns empty output (silent failure)**, fall back to direct curl with the token from `~/.mcp_servers.json`. See `references/mcp-cli-silent-failure.md`.
- For simple reads, curl via Path B works fine in cron contexts.
